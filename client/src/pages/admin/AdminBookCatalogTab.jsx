...
const AdminBookCatalogTab = () => {
  ...
  const fetchBooks = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/admin/books${showDeletedBooks ? '?deleted=true' : ''}`);
      setBooks(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingBook) {
        await api.put(`/admin/books/${editingBook._id}`, payload);
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h2 className="section-title" style={{ margin: 0 }}>Book Catalog</h2>
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={() => { setEditingBook(null); setFormData({ title: '', author: '', isbn: '', category: '', description: '', coverImage: '', publisher: '', publishedYear: '', language: 'English', totalCopies: 1, availableCopies: 1 }); setIsModalOpen(true); }} className="btn btn-primary btn-sm" style={{ padding: '0.5rem 1rem', borderRadius: '24px', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <FaPlus /> Add New Book
          </button>
          <button onClick={() => setShowDeletedBooks(!showDeletedBooks)} className={`btn ${showDeletedBooks ? 'btn-primary' : 'btn-outline'} btn-sm`} style={{ padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: '24px' }}>
...
export default AdminBookCatalogTab;
