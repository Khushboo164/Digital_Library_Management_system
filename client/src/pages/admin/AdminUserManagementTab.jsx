        ...
        <div className="modal-bg" onClick={() => { setAnalytics(null); setMemberId(''); }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
            <div className="panel" style={{ padding: '1.25rem', textAlign: 'center' }}>
              ...
              <h3 style={{ margin: 0, fontSize: '1.5rem' }}>{analytics.totalBorrows}</h3>
              <p className="text-muted" style={{ margin: 0 }}>Total Borrows</p>
            </div>
            <div className="panel" style={{ padding: '1.25rem', textAlign: 'center' }}>
              <FaBook style={{ color: '#10b981', fontSize: '1.5rem', marginBottom: '0.5rem' }} />
              <h3 style={{ margin: 0, fontSize: '1.5rem' }}>{analytics.currentlyBorrowed}</h3>
              <p className="text-muted" style={{ margin: 0 }}>Currently Borrowed</p>
            </div>
            <div className="panel" style={{ padding: '1.25rem', textAlign: 'center' }}>
              <FaCoins style={{ color: '#f59e0b', fontSize: '1.5rem', marginBottom: '0.5rem' }} />
              <h3 style={{ margin: 0, fontSize: '1.5rem' }}>₹{analytics.totalFines || 0}</h3>
              <p className="text-muted" style={{ margin: 0 }}>Total Fines</p>
            </div>
            <div className="panel" style={{ padding: '1.25rem', textAlign: 'center' }}>
              <FaExclamationTriangle style={{ color: '#ef4444', fontSize: '1.5rem', marginBottom: '0.5rem' }} />
              <h3 style={{ margin: 0, fontSize: '1.5rem' }}>{analytics.overdueBooks || 0}</h3>
              <p className="text-muted" style={{ margin: 0 }}>Overdue Books</p>
            </div>
          </div>
          ...
          <div className="panel" style={{ padding: '1.5rem' }}>
            ...
            {(!analytics.borrowHistory || analytics.borrowHistory.length === 0) ? (
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {analytics.borrowHistory.map(borrow => (
                      <tr key={borrow._id}>
                        <td><strong>{borrow.book?.title || 'Unknown'}</strong></td>
                        <td>{borrow.issueDate || borrow.requestDate ? new Date(borrow.issueDate || borrow.requestDate).toLocaleDateString() : 'N/A'}</td>
                        ...
                        <td>
                          <span className={`badge ${getStatusBadgeClass(borrow.status)}`} style={{ padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 600 }}>
                            {borrow.status || (borrow.returned ? 'Returned' : 'Borrowed')}
                          </span>
                        </td>
                        <td>{borrow.fine > 0 ? `₹${borrow.fine}` : '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
