...
import { FaBook, FaUsers, FaCoins, FaSync, FaSignOutAlt, FaChartBar, FaExchangeAlt, FaUndo, FaUserCheck, FaUserSlash, FaEnvelope, FaHistory, FaUser, FaUserEdit, FaBookOpen } from 'react-icons/fa';
...
import LibrarianCatalogTab from './LibrarianCatalogTab';
import LibrarianBorrowWorkspace from './LibrarianBorrowWorkspace';
import LibrarianReservationsTab from './LibrarianReservationsTab';
import LibrarianOverdueTab from './LibrarianOverdueTab';
import LibrarianPaymentsTab from "./LibrarianPaymentsTab";
import LibrarianNotificationsTab from "./LibrarianNotificationsTab";
import LibrarianMemberAnalyticsTab from "./LibrarianMemberAnalyticsTab";
import LibrarianEmailHistoryTab from "./LibrarianEmailHistoryTab";
import LibrarianLostBooksTab from "./LibrarianLostBooksTab";
import LibrarianMyActivitiesTab from "./LibrarianMyActivitiesTab";
import LibrarianDashboardOverview from "./LibrarianDashboardOverview";
...
const LibrarianDashboard = () => {
  ...
  useEffect(() => {
    if (activeTab === "overview" || activeTab === "view-profile") {
      if (!dashboardData) {
        fetchDashboardData();
      }
    }
  ...
  const navItems = [
    ...
    { id: "member-analytics", label: "Members", icon: <FaUsers /> },
    { id: "overdue", label: "Overdue Members", icon: <FaUserSlash /> },
    { id: "payments", label: "Fines & Replacement", icon: <FaCoins /> },
    { id: "lost-books", label: "Lost Books", icon: <FaBook /> },
    { id: "email-history", label: "Email History", icon: <FaEnvelope /> },
    { id: "notifications", label: "Notifications", icon: <FaSync /> },
  ];
      ...
      <aside className="dash-sidebar">
        <div className="dash-sidebar-brand" onClick={() => setActiveTab('overview')} style={{ cursor: 'pointer' }}>
          ...
          <div>
            ...
            <span className="brand-sub">Librarian Portal</span>
          </div>
        </div>
        ...
        <div className="dash-user-card">
          ...
          <div className="dash-user-meta">
            <h3>{user.name || 'Librarian'}</h3>
            <span className="dash-user-role">Librarian</span>
          </div>
        ...
        <header className="dash-topbar">
          <div className="topbar-left">
            ...
            <h3 className="topbar-title">
              {activeTab === "overview" && "Librarian Dashboard"}
              {activeTab === "my-activities" && "My Activities"}
              {activeTab === "catalog" && "Book Catalog"}
              {activeTab === "workspace" && "Borrow Workspace"}
              {activeTab === "reservations" && "Reservations Queue"}
              {activeTab === "overdue" && "Overdue Members"}
              { activeTab === "payments" && "Fines & Replacements" }
              { activeTab === "lost-books" && "Lost Books History" }
              { activeTab === "notifications" && "Notifications Center" }
              { activeTab === "member-analytics" && "Member Profile" }
              {activeTab === "email-history" && "Email History"}
              {activeTab === "profile" && "Update Profile"}
              {activeTab === "view-profile" && "Librarian Profile"}
            </h3>
          </div>
          ...
          {loading && activeTab === "overview" ? (
            ...
            <>
              ...
              {activeTab === "overview" && dashboardData && (
                <LibrarianDashboardOverview data={dashboardData} setActiveTab={setActiveTab} />
              )}

              {/* TABS */}
              {activeTab === "my-activities" && <LibrarianMyActivitiesTab setActiveTab={setActiveTab} />}
              {activeTab === "catalog" && <LibrarianCatalogTab />}
              {activeTab === "workspace" && <LibrarianBorrowWorkspace />}
              {activeTab === "reservations" && <LibrarianReservationsTab />}
              {activeTab === "overdue" && <LibrarianOverdueTab />}
              {activeTab === "payments" && <LibrarianPaymentsTab />}
              {activeTab === "lost-books" && <LibrarianLostBooksTab />}
              {activeTab === "notifications" && <LibrarianNotificationsTab />}
              {activeTab === "member-analytics" && <LibrarianMemberAnalyticsTab />}
              {activeTab === "email-history" && <LibrarianEmailHistoryTab />}
                ...
                <div className="animate-fade-in-up">
                  <div className="settings-card" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'center', padding: '2rem 1.5rem', maxWidth: '500px', margin: '0 auto' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                              <span className="badge badge-primary" style={{ fontSize: '0.9rem', padding: '0.5rem 1rem' }}>LIBRARIAN</span>
                    ...
                    <div style={{ width: '100%', background: 'var(--bg-muted)', padding: '1.5rem', borderRadius: '20px', display: 'flex', flexDirection: 'column', gap: '1.25rem', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
                            ...
                            <div style={{ background: 'white', padding: '1rem 0.75rem', borderRadius: '14px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}>
                                ...
                                <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-dark)' }}>{dashboardData?.overview?.totalBooksEntered || 0}</div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 600 }}>Total Books</div>
                            </div>
                            <div style={{ background: 'white', padding: '1rem 0.75rem', borderRadius: '14px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}><FaUndo /></div>
                                <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-dark)' }}>₹{(dashboardData?.overview?.pendingFine || 0) + (dashboardData?.overview?.pendingReplacementCost || 0)}</div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 600 }}>Pending Dues</div>
                            </div>
                            <div style={{ background: 'white', padding: '1rem 0.75rem', borderRadius: '14px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}><FaCoins /></div>
                                <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-dark)' }}>₹{dashboardData?.overview?.totalFineCollected || 0}</div>
                                <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 600 }}>Fines Collected</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        style={{ padding: '0.85rem 1.5rem', fontSize: '1rem', borderRadius: '12px', marginTop: '0.5rem', boxShadow: '0 6px 16px rgba(139, 92, 246, 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', width: '100%', background: 'var(--primary)', color: 'white' }}
                    >
