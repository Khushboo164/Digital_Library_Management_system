    ...
    <div className="animate-fade-in-up">
      ...
      <div className="flex gap-4 mb-4" style={{ flexWrap: 'wrap' }}>
        <div className="fine-filter-tabs" style={{ margin: 0 }}>
          ...
          <button onClick={() => setPaymentTypeTab("Late Fees")} className={`fine-filter-tab ${paymentTypeTab === "Late Fees" ? "active" : ""}`}>Late Fees</button>
          <button onClick={() => setPaymentTypeTab("Replacement Cost")} className={`fine-filter-tab ${paymentTypeTab === "Replacement Cost" ? "active" : ""}`}>Replacement Cost</button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6" style={{ flexWrap: 'wrap' }}>
        ...
        <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="pill-select" style={{ minWidth: '130px' }}>
          ...
          {['January', 'February', 'March', 'April', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map((m, i) => <option key={i+1} value={i+1}>{m}</option>)}
        </select>
...
export default LibrarianPaymentsTab;
