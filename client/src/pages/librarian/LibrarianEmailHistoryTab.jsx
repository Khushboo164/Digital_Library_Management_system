...
const LibrarianEmailHistoryTab = () => {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const res = await api.get('/librarian/email-history');
      setHistory(res.data);
    } catch (err) {
  ...
  const filteredHistory = history.filter(item => {
    ...
    // Type Filter
    if (activeTypeTab === "sent") {
      const userId = user.id || user._id;
      if (item.sentBy?._id !== userId && item.sentBy !== userId) {
        return false;
      }
    } else if (activeTypeTab === "received") {
      if (item.memberEmail !== user.email) {
        return false;
      }
    }
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="fine-filter-tabs" style={{ marginBottom: '1.25rem' }}>
        <button onClick={() => setActiveTypeTab("all")} className={`fine-filter-tab ${activeTypeTab === "all" ? "active" : ""}`}>All</button>
        <button onClick={() => setActiveTypeTab("sent")} className={`fine-filter-tab ${activeTypeTab === "sent" ? "active" : ""}`}>Sent</button>
        <button onClick={() => setActiveTypeTab("received")} className={`fine-filter-tab ${activeTypeTab === "received" ? "active" : ""}`}>Received</button>
      </div>
...
export default LibrarianEmailHistoryTab;
